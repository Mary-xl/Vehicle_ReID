...
def generator_batch_multitask(data_list, nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
    ...
    Input:
        这里输入的应该是所有的文本内容，是列表的形式，每个元素以换行符作为分隔符，每个元素的格式是： 图像地址 模型编号  颜色编号
        data_list  : a MxNet styple of data list, e.g.
                     "/data/workspace/dataset/Cervical_Cancer/train/Type_1/0.jpg 0"
        shuffle    : whether shuffle rows in the data_llist
        batch_size : batch size
    ...
    batch_index = 0
    while True:# 不断生成数据
        current_index = (batch_index * batch_size) % N # 防止越界
        if N >= (current_index + batch_size): # 如果当前批次没有越界
            current_batch_size = batch_size
            batch_index += 1
        else: # 如果当前批次越界
            current_batch_size = N - current_index
            batch_index = 0
            if shuffle: # 在每轮结束时，打乱数据
                random.shuffle(data_list)

        X_batch = np.zeros((current_batch_size, img_width, img_height, 3)) # 一张图，三个通道
        Y_batch_one = np.zeros((current_batch_size, nbr_class_one)) # onehot
        Y_batch_two = np.zeros((current_batch_size, nbr_class_two)) # onehot

        for i in range(current_index, current_index + current_batch_size):
            line = data_list[i].strip().split(' ') # 第i个样本
            #print line
            img_path = line[0] # 图片地址

            if random_scale:
                scale_ratio = random.uniform(0.9, 1.1)

            # 读图。经过这个函数后，img是一个内存中的图像，不再是一个地址字符串
            img = scale_byRatio(img_path, ratio=scale_ratio, return_width=img_width,
                                crop_method=crop_method)
            # 生成X_batch
            X_batch[i - current_index] = img

            # 生成Y_batch_one  Y_batch_two。经过这里的处理后，由编号变成独热编码了。
            if return_label:
        ...
        X_batch = X_batch.astype(np.float64)# 转换为浮点型，因为后面需要做标准化
        X_batch = preprocess_input(X_batch)
...
def generator_batch_triplet(data_list, dic_data_list, nbr_class_one=250, nbr_class_two=7,
    ...
    Input:
        ...
        mode       : generator used as for 'train', 'val'.
                     ...
                     if mode is set 'val', dic_data_list could be a null dictionary: { }.因为不用计算损失，不用找negative/positive
    ...
    while True:# 每个批次循环
        ...
        # Y_batch_fake ： identity_loss(y_true, y_pred): 占位用的，api固定格式，都是0

         # 某个批次的样本进行循环，生成数据
        for i in range(current_index, current_index + current_batch_size):
            line = data_list[i].strip().split(' ') # 某一行数据
            #print(line)
            ...
            if mode == 'train':
                ...
                # vehicleID are positives.
                # 找negative
                # Negatives are the ones with same modelID,
                # the same colorID but different vehicleIDs.
                # Same modelID but different colorIDs may also be considered as negatives.(这一条与代码不符)

                # 如果一辆车只有一张图片 说明anchor就没有positive
                ...
                # anchor的positive是车型、颜色、车相同的图片
                # copy a list of image paths with same vehicleID
                positive_list = dic[modelID][colorID][vehicleID][:] # 这里是所有的positive,包括anchor
                # 从positive中删去anchor
                ...
                # x_batch 为什么三个同样的？
                # 验证过程中不需要计算这个损失，所以是全部三个一样的，实际上只用到了一个车型、颜色分支，anchor、positive、negative因为网络结果存在但其实是不需要的。
                X_anchor[i - current_index] = anchor
...
def generator_batch_classware(data_list_ori, nbr_classes=3, batch_size=32, return_label=True,
    ...
    Output:
        (X_batch, Y_batch)
    '''
    data_list = [[] for i in range(nbr_classes)] # 车型？只有三个？3个空列表？
    for line in data_list_ori:
        label_temp = line.strip().split(' ')[-1] # 车型标签
        data_list[int(label_temp)].append(line) #某个车型加数据？

    N = np.zeros(nbr_classes, int) # 每个车型的数量?
    for i in range(nbr_classes):
        N[i] = len(data_list[i])
    index = np.zeros(nbr_classes, int)

    num_per_label = batch_size // nbr_classes # 每个批次中 每种车型能分到多少数据？
    while True:
        X_batch = np.zeros((batch_size, img_width, img_height, 3)) # 样本
        Y_batch = np.zeros((batch_size, nbr_classes)) # 标记
        list_this_batch = []
        for i in range(nbr_classes):
            # 如果一轮结束，打乱数据
            ...
            # 如果新增一个batch后，一轮没有结束，添加数据到batch列表中
